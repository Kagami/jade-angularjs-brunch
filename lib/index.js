...
(function() {
  ...
  module.exports = JadeAngularJsCompiler = (function() {
    ...
    function JadeAngularJsCompiler(config) {
      ...
      this.separator = config.optimize ? "" : "\n\n";
    ...
    JadeAngularJsCompiler.prototype.setupModule = function(pair) {
      var content, moduleName, modulePath, virtualPath;
      this.preparePair(pair);
      moduleName = 'partials';
      modulePath = [this["public"], 'static', 'js', "" + moduleName + ".js"].join(sysPath.sep);
      virtualPath = "/" + moduleName + "/" + (pair.path.slice(2).join('/'));
      content = pair.result;
      return {
    ...
    JadeAngularJsCompiler.prototype.writeModules = function(modules) {
      ...
      for (moduleName in modules) {
        ...
        content = ["angular.module('" + moduleName + "',[])", ".run(['$templateCache',function(t){", this.separator];
        templates.map(function(e) {
          return content.push("t.put('" + e.virtualPath + "','" + e.content + "');" + _this.separator);
        });
        content.push("}]);");
